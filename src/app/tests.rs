...
#[test]
fn only_a_hydrated_renoa_session_may_apply_a_replay() {
    assert!(can_apply_session_replay(ProviderKind::Renoa, true));
    // An unhydrated skeleton is not an empty cache; a replay arriving before
    // hydration must stay buffered, never applied against empty rows.
    assert!(!can_apply_session_replay(ProviderKind::Renoa, false));
    assert!(!can_apply_session_replay(ProviderKind::Grok, true));
}
