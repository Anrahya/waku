...
pub(crate) fn start_local(
    ...
    let inner: Arc<dyn DriverControl> = match provider {
        ...
        // Cursor, Fx, Grok, Kimi Code, and Renoa all serve a long-lived ACP
        // session, which is the only way their Supervised mode can actually
        // ask the user rather than silently forcing or denying.
        ProviderKind::Cursor
        | ProviderKind::Fx
        | ProviderKind::Grok
        | ProviderKind::Kimi
        | ProviderKind::Renoa => Arc::new(acp::AcpDriver::start(provider, options, events)?),
